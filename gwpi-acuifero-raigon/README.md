...
| Carpeta | Contenido |
|---|---|
| [`resultados/`](./resultados) | Productos del flujo operativo en ArcGIS Pro: diagrama de ModelBuilder, detalle de conversión a ráster, orden de capas, preparación del layout y vista general del proyecto SIG |
| [`scripts/`](./scripts) | Componente cuantitativa en Python puro: pesos AHP por el método del autovector principal, reclasificación por Rupturas Naturales y suma ponderada, con tests unitarios |
| [`notebooks/`](./notebooks) | Etapa vectorial previa, reproducible de punta a punta: de las capas crudas a los ratings 1–5, con manejo de sistemas de referencia, malla de análisis, densidad de drenaje, distancia a cursos, unidad geológica dominante, estadística zonal, validación por caudales de pozos y exportación a GeoPackage y GeoTIFF |
...
La preparación vectorial que antecede a la reclasificación se documenta de forma ejecutable en [`notebooks/`](./notebooks): reproyección a SIRGAS-ROU98 / UTM 21S (EPSG:5382), construcción de la malla de análisis, densidad de drenaje por intersección, distancia a cursos por unión espacial de vecino más próximo, asignación de unidad geológica por mayoría de área —con registro de la fracción de área como medida de ambigüedad— y estadística zonal por rasterización de identificadores. La verificación de consistencia de la matriz y la suma ponderada se resuelven en [`scripts/`](./scripts).
...
- Python: GeoPandas, Shapely, pyproj, Rasterio, NumPy, mapclassify, SciPy — preparación vectorial, estadística zonal, ponderación y exportación a GeoPackage y GeoTIFF
